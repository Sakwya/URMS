	...
	<el-container>
		...
		<el-container>
			...
			<el-main>
				...
				<el-dialog title="分配菜单" v-model="assignVisable" width="20rem"
					:style="{borderRadius:'10px',border:'0.125rem solid var(--el-color-primary)'}">
					<el-checkbox-group v-model="checkList">
						<el-checkbox v-for="(item,index) in menu" :label="item.name"></el-checkbox>
					</el-checkbox-group>
					<template #footer>
						<span>
...
<script setup>
	...
	const save = () => {
		axios.post('role', JSON.parse(JSON.stringify(formData.data)))
			.then(rspn => {
				ElMessage({
					type: 'success',
					message: "添加成功！",
	...
	const updata = () => {
		axios.post('role', JSON.parse(JSON.stringify(editData.data)))
			.then(rspn => {
				ElMessage({
					type: 'success',
					message: "修改成功！",
	...
	const assign = (id) => {
		rid = id
		axios(`menu`).then(rspn => {
			menu.value = rspn.data.data
			let menus = rspn.data.data;
			mid = {}
			rname = {}
			for (let i = 0; i < menus.length; i++) {
				let menu = menus[i]
				mid[menu['name']] = menu['id']
				rname[menu['id']] = menu['name']
			}
			axios(`role-menu/${id}`).then(rspn => {
				if (rspn.data.data != null) {
					let menus = rspn.data.data
					originmenu = []
					for (let i = 0; i < menus.length; i++) {
						originmenu[i] = rname[menus[i].mid]
					}
					checkList.value = originmenu
				}
			}).catch(error => {
				ElMessage({
					type: 'error',
